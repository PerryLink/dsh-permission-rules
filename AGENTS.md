...
- `test/` — vitest; real `Context` + real `Session`/`Commands`/`ApprovalService` from the `0.1.0-rc.8` peers; chokidar mocked with a fake EventEmitter; the dsh-auto-review integration uses its tarball with a scripted reviewer mock.
