...
- **The settings edit surface is therefore in-place instead of section-backed.** An edit is committed into the running references WITHOUT remounting the plugin, and the runtime's config source stays LAZY — `resolveConfig` unwraps the live references on every call, so a committed change is visible to the very next read rather than being frozen at mount. Because a volatile-only change does not remount, the new `loader/volatile-update` listener reconciles the bind-time side effects: it rebinds the local proxy when `network.proxyPort`, `proxyBind`, `injectEnv` or `noProxy` changed — the same four knobs the removed `register(..., { applies: 'live' })` watch callback served. Every other field is read live. `network` and `builtin` are marked volatile as WHOLE objects, which is the only shape Schemastery allows (it refuses a volatile inside a volatile).
- **The "an out-of-range value is refused at write time" tradeoff is preserved, by a different mechanism.** Schemastery 3.18.3 has no `.check()` (the cookbook names it, but it does not exist in the published package), so the numeric bounds now live in the SCHEMA (`min`/`max`/`step(1)`), which the config editor resolves the merged config against before it persists anything; the closed enums and boolean flags were already schema-level. The one rule Schemastery cannot express — `searchUp` combined with an absolute `rulesFile` — stays a `resolveConfig` failure at mount and on read, and the `loader/volatile-update` listener reports it in the plugin's own vocabulary instead of letting the Loader swallow a listener throw into a generic warning.
- The volatile marker is applied through a **capability probe**, not assumed. `.volatile()` first appeared in `@deepseek-ai/schemastery` 3.18.3 and this schema is built while the plugin module is evaluated, so an unguarded call would turn every host line the peer ranges still advertise into a hard mount crash. On a Schemastery without the method the fields stay ordinary values, `apply` receives a plain config, and the plugin runs exactly as before — only the live config form is unavailable, the same degradation as a host that composes no settings service.
- The `@deepseek-ai/dsh-agent-presets` → `@deepseek-ai/dsh-agent-preset-registry` rename needed no change here: this package imports neither name, and its `agentPreset` use is the session-header field, whose service key (`agentPresets`) is unchanged. `agent-preset/selected` is still a live session event type.
...
- `test/settings-page.spec.ts` now drives the shipped live path: the references are the real ones the schema produces and updates are committed through the same cross-realm protocol symbol the Loader's `updateVolatile` uses, so laziness is proven by committing an edit and re-reading the SAME config-source closure rather than a rebuilt one.
- `test/config.spec.ts` gains the write-time bound suite — the replacement for the removed `validate:` callback — plus a case pinning the one rule the schema cannot express.
