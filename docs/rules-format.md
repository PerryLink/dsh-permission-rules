...
| `ips` | 精确 IPv4/IPv6 字面量、glob（`10.0.*.*`）或 IPv4 CIDR（`10.0.0.0/8`）。URL 中的字面量 IP 始终是候选；代理额外解析主机名并测试其地址，随后**按裁决到的地址建连**（不做二次解析，DNS rebinding 无法把连接改到未获批的地址；多地址主机按裁决顺序回退），而 `tools/pre-execute` 只匹配字面量 IP（热路径不做 DNS）。IPv4 映射的 IPv6 字面量（`::ffff:169.254.169.254`、`::ffff:a9fe:a9fe`、`0:0:0:0:0:ffff:a9fe:a9fe`）归一化为其 IPv4 点分形式参与匹配，规则写成映射形式时同样覆盖普通 IPv4 写法。配置 `network.upstreamProxy` 后，链式请求的 DNS 由上游解析，因此 `ips` 作用域的裁决有意不链式，仍按裁决到的地址建连。任一命中即满足。 |
...
- 设置页的**放行**动作会在真正裁决该连接的最近一个规则文件的**最前面**插入一条 `match.network.domains` 放行规则（规则首个匹配生效，追加在既有匹配 `deny` 之后只会成为死文本），写入后立即重读规则链——含无会话宿主链——无需重启。
